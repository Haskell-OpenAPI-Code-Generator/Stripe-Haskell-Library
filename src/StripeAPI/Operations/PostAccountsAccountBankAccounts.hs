...
-- | > POST /v1/accounts/{account}/bank_accounts
--
-- The same as 'postAccountsAccountBankAccounts' but accepts an explicit configuration.
postAccountsAccountBankAccountsWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostAccountsAccountBankAccountsRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostAccountsAccountBankAccountsResponse)
postAccountsAccountBankAccountsWithConfiguration
  config
  account
  body =
...
-- | > POST /v1/accounts/{account}/bank_accounts
--
-- The same as 'postAccountsAccountBankAccounts' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postAccountsAccountBankAccountsWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
