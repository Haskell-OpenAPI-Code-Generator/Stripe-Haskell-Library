...
-- | > POST /v1/topups/{topup}
--
-- The same as 'postTopupsTopup' but accepts an explicit configuration.
postTopupsTopupWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostTopupsTopupRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostTopupsTopupResponse)
postTopupsTopupWithConfiguration
  config
  topup
  body =
...
-- | > POST /v1/topups/{topup}
--
-- The same as 'postTopupsTopup' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postTopupsTopupWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
