...
-- | > POST /v1/refunds/{refund}
--
-- The same as 'postRefundsRefund' but accepts an explicit configuration.
postRefundsRefundWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostRefundsRefundRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostRefundsRefundResponse)
postRefundsRefundWithConfiguration
  config
  refund
  body =
...
-- | > POST /v1/refunds/{refund}
--
-- The same as 'postRefundsRefund' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postRefundsRefundWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
