...
instance Data.Aeson.ToJSON PostAccountsAccountCapabilitiesCapabilityParameters where
...
instance Data.Aeson.ToJSON PostAccountsAccountCapabilitiesCapabilityRequestBody where
...
-- | > POST /v1/accounts/{account}/capabilities/{capability}
--
-- The same as 'postAccountsAccountCapabilitiesCapability' but accepts an explicit configuration.
postAccountsAccountCapabilitiesCapabilityWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostAccountsAccountCapabilitiesCapabilityRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostAccountsAccountCapabilitiesCapabilityResponse)
postAccountsAccountCapabilitiesCapabilityWithConfiguration
  config
  parameters
  body =
...
postAccountsAccountCapabilitiesCapabilityRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | Contains all available parameters of this operation (query and path parameters)
  PostAccountsAccountCapabilitiesCapabilityParameters ->
...
-- | > POST /v1/accounts/{account}/capabilities/{capability}
--
-- The same as 'postAccountsAccountCapabilitiesCapability' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postAccountsAccountCapabilitiesCapabilityWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
