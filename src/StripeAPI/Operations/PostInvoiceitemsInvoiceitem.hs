...
-- | > POST /v1/invoiceitems/{invoiceitem}
--
-- The same as 'postInvoiceitemsInvoiceitem' but accepts an explicit configuration.
postInvoiceitemsInvoiceitemWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostInvoiceitemsInvoiceitemRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostInvoiceitemsInvoiceitemResponse)
postInvoiceitemsInvoiceitemWithConfiguration
  config
  invoiceitem
  body =
...
-- | > POST /v1/invoiceitems/{invoiceitem}
--
-- The same as 'postInvoiceitemsInvoiceitem' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postInvoiceitemsInvoiceitemWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
