...
-- | > POST /v1/issuing/settlements/{settlement}
--
-- The same as 'postIssuingSettlementsSettlement' but accepts an explicit configuration.
postIssuingSettlementsSettlementWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostIssuingSettlementsSettlementRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostIssuingSettlementsSettlementResponse)
postIssuingSettlementsSettlementWithConfiguration
  config
  settlement
  body =
...
-- | > POST /v1/issuing/settlements/{settlement}
--
-- The same as 'postIssuingSettlementsSettlement' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postIssuingSettlementsSettlementWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
