...
instance Data.Aeson.ToJSON PostSourcesSourceVerifyRequestBody where
...
-- | > POST /v1/sources/{source}/verify
--
-- The same as 'postSourcesSourceVerify' but accepts an explicit configuration.
postSourcesSourceVerifyWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  PostSourcesSourceVerifyRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostSourcesSourceVerifyResponse)
postSourcesSourceVerifyWithConfiguration
  config
  source
  body =
...
-- | > POST /v1/sources/{source}/verify
--
-- The same as 'postSourcesSourceVerify' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postSourcesSourceVerifyWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
