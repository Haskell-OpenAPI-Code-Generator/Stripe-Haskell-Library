...
deleteEphemeralKeysKey ::
  ...
  StripeAPI.Common.StripeT m (Network.HTTP.Client.Types.Response DeleteEphemeralKeysKeyResponse)
deleteEphemeralKeysKey
  key
  body =
    GHC.Base.fmap
...
instance Data.Aeson.ToJSON DeleteEphemeralKeysKeyRequestBody where
...
-- | > DELETE /v1/ephemeral_keys/{key}
--
-- The same as 'deleteEphemeralKeysKey' but accepts an explicit configuration.
deleteEphemeralKeysKeyWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe DeleteEphemeralKeysKeyRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response DeleteEphemeralKeysKeyResponse)
deleteEphemeralKeysKeyWithConfiguration
  config
  key
  body =
...
-- | > DELETE /v1/ephemeral_keys/{key}
--
-- The same as 'deleteEphemeralKeysKey' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
deleteEphemeralKeysKeyWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
