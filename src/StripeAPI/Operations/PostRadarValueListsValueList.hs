...
-- | > POST /v1/radar/value_lists/{value_list}
--
-- The same as 'postRadarValueListsValueList' but accepts an explicit configuration.
postRadarValueListsValueListWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostRadarValueListsValueListRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostRadarValueListsValueListResponse)
postRadarValueListsValueListWithConfiguration
  config
  valueList
  body =
...
-- | > POST /v1/radar/value_lists/{value_list}
--
-- The same as 'postRadarValueListsValueList' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postRadarValueListsValueListWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
