...
instance Data.Aeson.ToJSON GetCustomersCustomerDiscountParameters where
...
-- | > GET /v1/customers/{customer}/discount
--
-- The same as 'getCustomersCustomerDiscount' but accepts an explicit configuration.
getCustomersCustomerDiscountWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GetCustomersCustomerDiscountParameters ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response GetCustomersCustomerDiscountResponse)
getCustomersCustomerDiscountWithConfiguration
  config
  parameters =
    GHC.Base.fmap
...
-- | > GET /v1/customers/{customer}/discount
--
-- The same as 'getCustomersCustomerDiscount' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
getCustomersCustomerDiscountWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
