...
-- | > DELETE /v1/subscriptions/{subscription_exposed_id}
--
-- The same as 'deleteSubscriptionsSubscriptionExposedId' but accepts an explicit configuration.
deleteSubscriptionsSubscriptionExposedIdWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe DeleteSubscriptionsSubscriptionExposedIdRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response DeleteSubscriptionsSubscriptionExposedIdResponse)
deleteSubscriptionsSubscriptionExposedIdWithConfiguration
  config
  subscriptionExposedId
  body =
...
-- | > DELETE /v1/subscriptions/{subscription_exposed_id}
--
-- The same as 'deleteSubscriptionsSubscriptionExposedId' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
deleteSubscriptionsSubscriptionExposedIdWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
