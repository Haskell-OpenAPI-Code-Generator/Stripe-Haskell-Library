...
-- | > POST /v1/account/persons/{person}
--
-- The same as 'postAccountPersonsPerson' but accepts an explicit configuration.
postAccountPersonsPersonWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostAccountPersonsPersonRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostAccountPersonsPersonResponse)
postAccountPersonsPersonWithConfiguration
  config
  person
  body =
...
-- | > POST /v1/account/persons/{person}
--
-- The same as 'postAccountPersonsPerson' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postAccountPersonsPersonWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
