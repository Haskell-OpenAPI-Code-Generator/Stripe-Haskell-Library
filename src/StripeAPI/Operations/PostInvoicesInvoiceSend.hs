...
postInvoicesInvoiceSend ::
  ...
  StripeAPI.Common.StripeT m (Network.HTTP.Client.Types.Response PostInvoicesInvoiceSendResponse)
postInvoicesInvoiceSend
  invoice
  body =
    GHC.Base.fmap
...
instance Data.Aeson.ToJSON PostInvoicesInvoiceSendRequestBody where
...
-- | > POST /v1/invoices/{invoice}/send
--
-- The same as 'postInvoicesInvoiceSend' but accepts an explicit configuration.
postInvoicesInvoiceSendWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostInvoicesInvoiceSendRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostInvoicesInvoiceSendResponse)
postInvoicesInvoiceSendWithConfiguration
  config
  invoice
  body =
...
-- | > POST /v1/invoices/{invoice}/send
--
-- The same as 'postInvoicesInvoiceSend' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postInvoicesInvoiceSendWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
