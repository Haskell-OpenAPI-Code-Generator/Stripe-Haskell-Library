...
instance Data.Aeson.ToJSON PostSubscriptionSchedulesScheduleCancelRequestBody where
...
-- | > POST /v1/subscription_schedules/{schedule}/cancel
--
-- The same as 'postSubscriptionSchedulesScheduleCancel' but accepts an explicit configuration.
postSubscriptionSchedulesScheduleCancelWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostSubscriptionSchedulesScheduleCancelRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostSubscriptionSchedulesScheduleCancelResponse)
postSubscriptionSchedulesScheduleCancelWithConfiguration
  config
  schedule
  body =
...
postSubscriptionSchedulesScheduleCancelRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | schedule | Constraints: Maximum length of 5000
  Data.Text.Internal.Text ->
...
-- | > POST /v1/subscription_schedules/{schedule}/cancel
--
-- The same as 'postSubscriptionSchedulesScheduleCancel' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postSubscriptionSchedulesScheduleCancelWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
