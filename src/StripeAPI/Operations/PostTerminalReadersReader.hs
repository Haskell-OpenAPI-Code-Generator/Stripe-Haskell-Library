...
-- | > POST /v1/terminal/readers/{reader}
--
-- The same as 'postTerminalReadersReader' but accepts an explicit configuration.
postTerminalReadersReaderWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostTerminalReadersReaderRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostTerminalReadersReaderResponse)
postTerminalReadersReaderWithConfiguration
  config
  reader
  body =
...
-- | > POST /v1/terminal/readers/{reader}
--
-- The same as 'postTerminalReadersReader' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postTerminalReadersReaderWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
