...
-- | > POST /v1/plans/{plan}
--
-- The same as 'postPlansPlan' but accepts an explicit configuration.
postPlansPlanWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostPlansPlanRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostPlansPlanResponse)
postPlansPlanWithConfiguration
  config
  plan
  body =
...
-- | > POST /v1/plans/{plan}
--
-- The same as 'postPlansPlan' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postPlansPlanWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
