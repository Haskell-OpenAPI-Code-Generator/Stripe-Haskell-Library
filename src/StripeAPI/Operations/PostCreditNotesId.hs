...
-- | > POST /v1/credit_notes/{id}
--
-- The same as 'postCreditNotesId' but accepts an explicit configuration.
postCreditNotesIdWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostCreditNotesIdRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostCreditNotesIdResponse)
postCreditNotesIdWithConfiguration
  config
  id
  body =
...
-- | > POST /v1/credit_notes/{id}
--
-- The same as 'postCreditNotesId' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postCreditNotesIdWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
