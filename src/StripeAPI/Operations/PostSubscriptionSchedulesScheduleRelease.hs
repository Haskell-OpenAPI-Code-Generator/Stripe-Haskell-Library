...
instance Data.Aeson.ToJSON PostSubscriptionSchedulesScheduleReleaseRequestBody where
...
-- | > POST /v1/subscription_schedules/{schedule}/release
--
-- The same as 'postSubscriptionSchedulesScheduleRelease' but accepts an explicit configuration.
postSubscriptionSchedulesScheduleReleaseWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostSubscriptionSchedulesScheduleReleaseRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostSubscriptionSchedulesScheduleReleaseResponse)
postSubscriptionSchedulesScheduleReleaseWithConfiguration
  config
  schedule
  body =
...
postSubscriptionSchedulesScheduleReleaseRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | schedule | Constraints: Maximum length of 5000
  Data.Text.Internal.Text ->
...
-- | > POST /v1/subscription_schedules/{schedule}/release
--
-- The same as 'postSubscriptionSchedulesScheduleRelease' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postSubscriptionSchedulesScheduleReleaseWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
