...
-- | > POST /v1/account/external_accounts
--
-- The same as 'postAccountExternalAccounts' but accepts an explicit configuration.
postAccountExternalAccountsWithConfiguration ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  ...
  GHC.Maybe.Maybe PostAccountExternalAccountsRequestBody ->
  -- | Monadic computation which returns the result of the operation
  m (Network.HTTP.Client.Types.Response PostAccountExternalAccountsResponse)
postAccountExternalAccountsWithConfiguration
  config
  body =
    GHC.Base.fmap
...
postAccountExternalAccountsRaw body = GHC.Base.id (StripeAPI.Common.doBodyCallWithConfigurationM (Data.Text.toUpper GHC.Base.$ Data.Text.pack "POST") (Data.Text.pack "/v1/account/external_accounts") [] body StripeAPI.Common.RequestBodyEncodingFormData)

-- | > POST /v1/account/external_accounts
--
-- The same as 'postAccountExternalAccounts' but accepts an explicit configuration and returns the raw 'Data.ByteString.Char8.ByteString'.
postAccountExternalAccountsWithConfigurationRaw ::
  forall m.
  StripeAPI.Common.MonadHTTP m =>
  -- | The configuration to use in the request
  StripeAPI.Common.Configuration ->
